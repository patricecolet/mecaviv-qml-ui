...
Mac → Raspberry Pi over SSH, systemd `--user` on device. Details — including the traps —
live in `deploy/CLAUDE.md`, which loads when you work under `deploy/`, and `deploy/README.md`.
...
- **Inbound binary** = raw MIDI, 1 or 3 bytes, forwarded to `MidiMonitorController.applyExternalMidiBytes`, which decodes and emits `midiDataChanged(note, velocity, bend, channel)`. `main.qml` routes it into `LiveState.applyMidi`, which keeps one entry per siren in `sirenMidi` and feeds `SirenRingRow2D` (note at the ring centre, pulse on attack).
  ...
  **The note on that bus is a semitone below what you hear, and that is by design.** `pd transposeQuarterTune` in `harmoniseur.pd` writes the pair `note = floor($f1 - 0.5 + $f2/8192)` / `bend = fmod($f2 + 4096, 8192)` — a C played with no bend becomes **59 with a bend of 4096**. Its own comment says why: "the tune is scaled 1/4 tone lower", and `composeSiren~` knows the convention, so the sound is correct. `$0.to.sirens` therefore does **not** carry musical notes, and reading the note alone as a MIDI pitch is meaningless. `pd midi.binary` adds 1 before sending to QML for exactly this reason (2026-08-31); anything else that reads this bus to *display* a pitch must do the same. Exact while the incoming bend is neutral — a played note; under a real bend the wire note drifts by up to a semitone, and making it exact would mean carrying the bend on the binary channel and recomposing `note + bend/8192 + 0.5`. **Do not "fix" the harmoniser for this** — standing instruction from Patrice.
