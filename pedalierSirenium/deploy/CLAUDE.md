...
`scripts/start.pedalier.sh` is now a deprecation stub, `scripts/testServerOnRasp.sh` is gone (`pedalier-deploy.sh build` replaces it). The Pi runs **Pd 0.55.2 from `/usr/local`** (Debian's 0.53 stays in `/usr/bin/pd` as a one-line fallback). `pdjson` needs the flat link `~/pd-externals/pdjson.pd_lua`: no Pd version applies the folder convention to `.pd_lua`. **pdlua is compiled** (tag in `manifest/pdlua-version.txt`, against `~/dev/src/pure-data`), not taken from Debian: the packaged 0.7.3 lacks `_canvaspath`, so pdjson is created fine and then fails every relative `read` — pedalier.pd boots without its compositions. The compiled pdlua embeds Lua 5.4, so lunajson must be visible under `/usr/local/share/lua/5.4/`. `pdjson_loads` therefore probes an actual `read`, not just object creation.
